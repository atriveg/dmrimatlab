% represent_dmri_ellipsoids.m
clear;

% This script illustrates a simple pipeline to read a DWI volume and
% represent DT-MRI ellipsoids at desired positions.
...
% Compute the SH decomposition of the ADC, i.e. diffusion ellipsoids:
SHodf = atti2shadc( atti, gi, bi, 'L', 2, 'lambda', 0.001, 'mask', mask );
...
[ha,ho] = plotdmri3d(SHodf,i1,i2,i3,'ha',ha, 'origin', origin, ...
    ...
    'angleres', 642, 'glyphspc', [1;1;1], 'glyphsc', 2.0, ...
    'glyphscvar', 'bg' );
light;
axis('equal');
rotate3d('on');
