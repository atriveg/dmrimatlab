function rtop = atti2artop( atti, gi, bi, varargin )
% function rtop = atti2artop( atti, gi, bi, 'option1', value1, ... )
%
%   Computes the (apparent) Return To Origin Probability according to a 
...
%   Outputs:
%
%      rtop: MxNxP double array with the computed RTOP.
%
...
% -------------------------------------------------------------------------
L = (sqrt(8*(G-1)+9)-3)/2; % Take the maximum available SH order
L = 2*floor(L/2);
rtop = atti2amura( atti, gi, bi, ...
    'type', 'rtop', 'mask', opt.mask, ...
