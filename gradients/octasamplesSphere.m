function [grads,linit,lend] = octasamplesSphere(L,varargin)
% function [grads,linit,lend] = octasamplesSphere(L,'opt1',val1,'opt2',val2...)
%
%    Creates (roughly) uniform samplings of the unit sphere based on
%    succesive refinements of an octahedron. The sampling is
...
%    brief:
%
%      - We start with an octahedron, i.e. 6 vertices and 8 facets. In
%        the first level, l=1, the gradients will be taken from the 6
%        vertices. However, since the octahedron is symmetric and two
%        opposite gradients represent the same diffusion measurement, only
%        3 gradients are returned.
%
%      - For level l=2, the gradient directions corresponding to the 8
%        facets are chosen. However, the ctual amount of diffusion
%        gradients is 4 since we have to remove opposites.
%
%      - If L>2, the octahedron is refiend by splitting each facet in 4
%        triangles, inserting new vertexes at the mid points of the
%        original edges (see refinePolyhedron.m). A grand total of 18
%        vertices is obtained this way, among which 12 of them are new
%        points (the remaining 6 where already in the original
%        octahedron). From these new 12 points, we keep 6 of them and
%        remove their opposites.
%
%      - If L>3, the center points of the newly introduced facets are used
%        as gradients: the original 8 facets are refined into 32 facets,
%        so that 32-8=24 new center points are obtained. Only 12 up of
%        these 24 points are actually used since the opposites must be
%        removed.
%
%      - This process is recurssively repeated if L>4 to obtain the
%        following hierarchy of interleaved gradient directions:
%
%      L:           1   2   3   4   5   6   7    ...   L
%      grads_L:     3   4   6   12  24  48  96   ...   2^(L-3)*6
%      total_grads: 3   7   13  25  49  97  193  ...   7+6*(2^(L-2)-1)
%
