...
x     = -[-0.99999999,-0.9999999,-0.999999,-0.99999,-0.9999,-0.999,-0.9,-0.5,0.0,0.12,0.77,0.999,0.999999,0.999999999];
...
for n=1:length(gamma)
    ...
    if(length(f1)~=length(x))
        ...
        % missbehavior:
        %f1 = [ nan, hypergeom([1/2,gamma(n)/2],3/2,x(2:end)) ];
        f1 = [ nan, gsl_sf_hyperg_2F1( 1/2, gamma(n)/2, 3/2, x(2:end) ) ];
    end
    f2 = dmri_2F1_( ...
        gamma(n), ...
        x );
