function [SH,A] = hot2sh( HOT, varargin )
% function [SH,A] = hot2sh( HOT, 'opt1', value1, 'opt2', value2, ... )
%
%   Computes the SH coefficients of a signal at each voxel in terms of the
%   (unique) components of a hyper-symmetric, higher order tensor of rank 
%   L.
%
%      HOT: a MxNxPx((L+1)(L+2)/2) double array containing the unique 
%         coefficients of the equivalent HOT representation. Note L is
%         even, and the fourth dimension of this array must match a 
%         proper size.
%      SH: a MxNxPx((L+1)(L+2)/2) double array containing the coefficients
%         of the spherical harmonics expansion at each voxel. 
%      A: if a second output is requested, A is the conversion matrix
%         leading from HOT to SH (Size KxK, for K=(L+1)(L+2)/2).
%
...
if(nargin<1)
    error('At lest the HOT coefficients volume must be supplied');
end
HOT = double(HOT);
[M,N,P,K] = size(HOT);
...
mask = opt.mask(:);          % NVx1
HOT  = HOT(mask,:); % PVxK
[SH2,A] = hot2sh_(HOT,opt.maxthreads);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SH = zeros(NV,K);
SH(mask,:) = SH2;
