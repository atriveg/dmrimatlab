...
case "$1" in
  "single-thread")
    make -j DYNAMIC_ARCH=0 BINARY=64 INTERFACE=32 NO_AFFINITY=1 NO_WARMUP=1 USE_OPENMP=0 USE_THREAD=0 USE_LOCKING=1 NUM_PARALLEL="$(nproc --all)" NO_CBLAS=1 NO_LAPACKE=1 LIBNAMESUFFIX="${LIBSUFFIX}"
    ;;
  *)
    make -j DYNAMIC_ARCH=0 BINARY=64 INTERFACE=32 NO_AFFINITY=1 NO_WARMUP=1 USE_OPENMP=0 USE_THREAD=0 USE_LOCKING=1 NUM_PARALLEL="$(nproc --all)" NO_CBLAS=1 NO_LAPACKE=1 LIBNAMESUFFIX="${LIBSUFFIX}"
    ;;
...
make DYNAMIC_ARCH=0 BINARY=64 INTERFACE=32 NO_AFFINITY=1 NO_WARMUP=1 USE_OPENMP=0 USE_THREAD=0 USE_LOCKING=1 NUM_PARALLEL="$(nproc --all)" NO_CBLAS=1 NO_LAPACKE=1 LIBNAMESUFFIX="${LIBSUFFIX}" PREFIX="${INSTALLDIR}" install
