...
function download_dmritestdata
...
try
    ...
    fname = dmrimatlab_websave(DFILE,[BASEURL,DFILE]);
    ...
    if(~strcmp(fname,DFILE))
        ...
        cd(oldpath);
        error('Something went wrong. File %s seems not to be accessible in the server [FATAL]',DFILE);
    end
catch ME
    try
        delete([DFILE,'.html']);
    ...
    cd(oldpath);
    fprintf(1,'Something went wrong. Couldn''t download test data [FATAL]\n');
    rethrow(ME);
end
...
function fname = dmrimatlab_websave(local,url)
sf = check_software_platform;
if(sf==1)
    fname = websave(local,url);
elseif(sf==2)
    fname = local;
    try
        [data,success,message] = urlread(url);
        if(~success)
            fprintf(1,'%s\n',message);
            error('urlread was unable to donwlonad the file');
        end
        try
            fid = fopen(fname,'w');
            fwrite(fid,data);
            ...
            error('Unable to write downloaded data to file');
        end
    catch ME
        fprintf(1,'Could not run download command\n');
        rethrow(ME);
    end
end
end
